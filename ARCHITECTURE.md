...
> Current canonical architecture documentation lives in `build-plans-architecture/ARCHITECTURE (4).md`. This root copy is retained for historical build-plan context and may contain superseded wording. Use the canonical file plus `README.md` and `build-plans-architecture/SUBMISSION_CHECKLIST_STATUS.md` for the current submission state, known limitations, AWS deployment plan, and checklist audit.
